...
if $AUTOPOINT0.18.3 >/dev/null 2>&1
then AC_POSTFIX=0.18.3
elif $AUTOPOINT >/dev/null 2>&1
then AC_POSTFIX=""
else
  echo 'you need autopoint (0.18.3+ recommended) to generate the Makefile'
  exit 1
fi
echo `autopoint$AC_POSTFIX --version | head -1` found
